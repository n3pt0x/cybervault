# MISC

> [!NOTE]
> The tools offered allow you to avoid giving data to data brokers, for example, and prevent you from having to log in to accounts and other sites.

## 🖧 Torrent

- [uTorrent](https://www.utorrent.com/) - Web torrent.
- [qBittorrent](https://www.qbittorrent.org/) - Client torrent.
- [webTorrent](https://webtorrent.io/) - Hosted your own Torrent website.
- [Instant.io](https://instant.io/) - Share & download torrent from web.

## 📱 Front End

- [invidious](https://invidious.nerdvpn.de/) - Alternative to youtube front-end.
